    ...
    class BubbleSortThread extends Thread{
        ...
        public void run() {
            while (!finished){
                ...
                int lastIndex = size - 1;if (reset == true) continue; finalPause();
                for (int i = 0; i < size; i++){
                    array[i] = oldarray[i];
                    textboxEqualUpdate(i, R.color.colorW);
                    keyboxColortUpdate(i, R.color.colorW);
                    textboxTextUpdate(i, array[i]);if (reset == true) continue;finalPause();
                    textboxColorUpdate(i, colorPrimary);if (reset == true) continue;finalPause();
//                    textViuSizeUpdate(i, array[i]);
                ...
                if (reset == true) continue;finalPause();
                int key = 5;if (reset == true) continue;finalPause();
                if (reset == true) continue;finalPause();
                binarySearch(array, key, 0, size - 1);if (reset == true) continue;finalPause();

                textlineColorUpdate(5, R.color.colorAccent);if (reset == true) continue;finalPause();
                waitsec(1);if (reset == true) continue;finalPause();
        ...
        void binarySearch(int[] array, int key, int left, int right){
            ...
            for (int i = left; i <= right; i++){
                textboxColorUpdate(i, R.color.colorB);if (reset == true) return;finalPause();
            }
            waitsec(1);if (reset == true) return;finalPause();
            textlineColorUpdate(3, R.color.colorAccent);
            int mid = (left + right) / 2;if (reset == true) return;finalPause();
            textboxColorUpdate(mid, R.color.colorAccent);if (reset == true) return;finalPause();
            keyboxColortUpdate(mid, R.color.colorPrimary);if (reset == true) return;finalPause();
            waitsec(1);if (reset == true) return;finalPause();
            ...
            waitsec(1);if (reset == true) return;finalPause();
            if (array[mid] == key) {
                textboxEqualUpdate(mid, R.color.colorPrimary);if (reset == true) return;finalPause();
                waitsec(1);if (reset == true) return;finalPause();
            ...
            waitsec(1);if (reset == true) return;finalPause();
            for (int i = left; i <= right; i++){
                textboxColorUpdate(i, R.color.colorPrimary);if (reset == true) return;finalPause();
            }
            textboxEqualUpdate(mid, R.color.colorW);if (reset == true) return;finalPause();
            keyboxColortUpdate(mid, R.color.colorW);if (reset == true) return;finalPause();
            waitsec(1);if (reset == true) return;finalPause();
